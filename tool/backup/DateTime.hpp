...
#else
	  bool setAllDateTime(std::string fileDate) {
      // 檔名分兩種  2017-12-16_030428  2017-12-16_07:36:39
      if (fileDate.size() == 17) {
        // 2017-12-16_030428
        // ----- 日期 -----
        if (!setDate(fileDate)) {
        ...
        // 檢查 12~17 個是不是數字
        int temp;
        if (StringToNumber(fileDate.substr(11,6), &temp)) {
          hour = temp/10000;       // hour
          minute = (temp/100)%100; // minute
          second = temp%100;       // second
        } else {
          return false;
        }
        return true;
      } else if (fileDate.size() == 20) {
        // 2017-12-16_07:36:39
        // ----- 日期 -----
        if (!setDate(fileDate)) {
          return false;
